...
SELECT
    "title" AS "films_R_rated",
    "rating" AS "film_rating" -- Añado "rating" para comprobar los resultados
FROM "film"
WHERE "rating" = 'R';

-- 3 - Actores cuyo ID está entre 30 y 40
SELECT 
    "actor_id" AS "id_between_30_and_40",
    CONCAT("first_name", ' ', "last_name") AS "actor_name" -- Añado "actor_id" para comprobar los resultados
FROM "actor"
WHERE "actor_id" between 30 and 40;

-- 4 - Películas cuyo idioma sea igual al original
SELECT 
    "title" AS "film_in_original_language",
    "language_id" AS "film_language",
    "original_language_id" AS "film_original_language" -- Añado "language_id" y "original_language_id" para comprobar los resultados
FROM "film"
...
SELECT
    "title" AS "film_title",
    "length" AS "film_length" -- Añado "length" para comprobar los resultados
FROM "film"
ORDER BY "length";
...
SELECT 
    "title" AS "films_PG_13_or_length>180",
    "rating" AS "film_rating",
    "length" AS "film_length" -- Añado "rating" y "length" para comprobar los resultados obtenidos
FROM "film"
WHERE ("rating" = 'PG-13') OR ("length" > 180);

-- 9 - Variabilidad de reemplazar las películas
SELECT
    MIN("replacement_cost") AS "film_min_replacement_cost",
    MAX("replacement_cost") AS "film_max_replacement_cost"
FROM "film";

-- 10 - Mayor y menor duración de películas
SELECT
    MIN("length") AS "film_min_length",
    MAX("length") AS "film_max_length"
FROM "film";

-- 11 - Coste antepenúltimo alquiler ordenado por día
SELECT
    "r"."rental_id" AS "thrid_last_rent",
    "rental_date" AS "third_last_date",
    "amount" AS "third_last_amount"
FROM "rental" AS "r"
...
SELECT
    "title" AS "film_title",
    "rating" AS "rating_not_NC_17_or_G"
FROM "film"
...
SELECT 
    "rating" AS "film_rating",
    ROUND(AVG("length")) AS "film_avg_length"
FROM "film" 
GROUP BY "rating";
...
SELECT
    "customer_id" AS "ten_higher_customer_id",
...
SELECT 
    CONCAT("first_name", ' ', "last_name") AS "actor_name_from_EGG_IGBY",
    "title" AS "film_title"
FROM "actor" AS "a" 
...
SELECT
    ...
    ROUND(AVG("length")) AS"film_length>110"
FROM "film" AS "f" 
    INNER JOIN "film_category" AS "fc" ON "f"."film_id" = "fc"."film_id" 
    INNER JOIN "category" AS "c" ON "fc"."category_id" = "c"."category_id"
GROUP BY "c"."name" 
...
SELECT
    ROUND(AVG(EXTRACT(DAY FROM "return_date"::TIMESTAMP - "rental_date"::TIMESTAMP)), 2) AS "avg_days_rented"
FROM "rental";
...
SELECT
    DATE_TRUNC('DAY', "rental_date") AS "day_date",
...
SELECT
    "title" AS "film_title",
    "length" AS "film_length_>avg"
FROM "film" 
WHERE "length" > (SELECT AVG("length") FROM "film");

-- 25 - Alquileres registrados por mes
SELECT
    DATE_TRUNC('MONTH', "rental_date") AS "rental_month_date",
...
SELECT
    "title" AS "film_title",
    "rental_rate" AS "film_rental_rate>avg"
FROM "film" 
WHERE 
...
SELECT 
    ...
    COUNT("film_id") AS "num_films_made"
FROM "actor" AS "a" 
...
SELECT
    "title" AS "film_title",
    COUNT("inventory_id") AS "qty_available"
FROM "film" AS "f"
...
SELECT 
    "title" AS "film_title",
    CONCAT("first_name", ' ', "last_name") AS "actor_in_film_by_name"
...
SELECT
    "title" AS "film_title",
    "rental_date" AS "rental_date_registered"
FROM "film" AS "f" 
    LEFT JOIN "inventory" AS "i" ON "f"."film_id" = "i"."film_id" 
...
SELECT
    "actor_id",
    CONCAT("first_name", ' ', "last_name") AS "actor_order_by_last_name"
FROM "actor"
WHERE
    ("actor_id" = (SELECT MIN("actor_id") FROM "actor")) OR 
    ("actor_id" = (SELECT MAX("actor_id") FROM "actor"));
...
SELECT CONCAT("last_name", ', ', "first_name") AS "actor_order_by_last_name"
FROM "actor"
ORDER BY "last_name";
...
SELECT
    "first_name" AS "actors_first_name",
    COUNT(DISTINCT "actor_id") AS "num_actors_with_that_name"
FROM "actor"
GROUP BY "first_name" 
...
-- porque ni siquiera sabemos a qué categoría corresponde cada película originalmente


-- 45 - Actores de la categoría Action
SELECT
    CONCAT("first_name", ' ', "last_name") AS "action_actor",
    COUNT("fa"."film_id") AS "num_action_films_made"
FROM "actor" AS "a"
    LEFT JOIN "film_actor" AS "fa" ON "a"."actor_id" = "fa"."actor_id"
...
SELECT
    "c"."name" AS "film_category_name",
    SUM("length") AS "action_films_total_length"
FROM "film" AS "f"
    INNER JOIN "film_category" AS "fc" ON "f"."film_id" = "fc"."film_id" 
    INNER JOIN "category" AS "c" ON "fc"."category_id" = "c"."category_id" 
WHERE "c"."name" = 'Action'
...
SELECT
    CONCAT("first_name", ' ', "last_name") AS "customer_name",
    "title" AS "films_not_return_yet"
FROM "customer" AS "c" 
    INNER JOIN "rental" AS "r" ON "c"."customer_id" = "r"."customer_id" 
    INNER JOIN "inventory" AS "i" on "r"."inventory_id" = "i"."inventory_id" 
    INNER JOIN "film" AS "f" ON "i"."film_id" = "f"."film_id" 
WHERE
    (CONCAT("first_name", ' ', "last_name") = 'TAMMY SANDERS') AND
	("return_date" = NULL)
ORDER BY "title";
...
SELECT
    ...
    ROUND(COUNT("fa"."film_id")) AS "num_films_made_by_sci_fi_actors"
FROM "actor" AS "a"
    LEFT JOIN "film_actor" AS "fa" ON "a"."actor_id" = "fa"."actor_id"
    LEFT JOIN "film_category" AS "fc" ON "fa"."film_id" = "fc"."film_id" 
    LEFT JOIN "category" AS "c" ON "fc"."category_id" = "c"."category_id" 
...
SELECT
    CONCAT("last_name", ', ', "first_name") AS "actor_name",
    COUNT("fa"."film_id") AS "films_rented_after_SPARTACUS_CHEAPER_first_rental" 
FROM "actor" AS "a"
    LEFT JOIN "film_actor" AS "fa" ON "a"."actor_id" = "fa"."actor_id"
    LEFT JOIN "inventory" AS "i" ON "fa"."film_id" = "i"."film_id"
    LEFT JOIN "rental" AS "r" ON "i"."inventory_id" = "r"."inventory_id"
WHERE
    "rental_date" > (SELECT "scfr" FROM "spartacus_cheaper_first_rental")
GROUP BY "a"."actor_id" 
ORDER BY "last_name";

-- 56 - Actores que no han actuado en peliculas cat 'Music'
SELECT CONCAT("first_name", ' ', "last_name") AS "actor_name_not_music"
FROM "actor"
...
SELECT
    "title" AS "film_title",
    EXTRACT(DAY FROM "return_date"::TIMESTAMP - "rental_date"::TIMESTAMP) AS "days_rented"
FROM "rental" AS "r"
    INNER JOIN "inventory" AS "i" ON "r"."inventory_id" = "i"."inventory_id"
    INNER JOIN "film" AS "f" ON "i"."film_id" = "f"."film_id"
...
SELECT
    "c"."name" AS "film_category_name",
    COUNT("release_year") AS "released_2006"
FROM "category" AS "c"
...
SELECT
    COUNT("rental_id") AS "films_rented",
    CONCAT("first_name", ' ', "last_name", '-',
    COUNT("rental_id")) AS "customer_info"
FROM "customer" AS "c" 
    LEFT JOIN "rental" AS "r" ON "c"."customer_id" = "r"."customer_id" 
GROUP BY "c"."customer_id";